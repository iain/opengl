...
describe Adder::World do
  ...
  it "repositions the spaceship with an acceleration of [2,5,3] calculated over 2 second" do
    ...
    spaceship.acceleration = Vector[2, 5, 3, 0]
    subject.over(2)
    spaceship.matrix.row_vectors.last.should == Vector[4, 10, 6, 1]
  end

  it "repositions the spaceship with a velocity of [2,3,3] calculated over 10 sec" do
    spaceship = Spaceship.new
    subject.bodies[:spaceship] = spaceship
    spaceship.velocity = Vector[2,3,3, 0]
    subject.over(10)
    spaceship.matrix.row_vectors.last.should == Vector[20,30,30,1]
  end

  it "repositions the spaceship with an acceleration of -9.8 in the y direction calculated over 3 second" do
    spaceship = Spaceship.new
    spaceship.acceleration = Vector[0, -9.8, 0, 0]
    subject.bodies[:spaceship] = spaceship
    subject.over(1)
    subject.over(2)
    spaceship.matrix.row_vectors.last.should be_vector_like Vector[0,-44.1,0,1]
  end

  it "rotates the spaceship with an angular acceleration of [3,2,2] over 3 sec" do
    spaceship = Spaceship.new
    spaceship.angular_acceleration = Vector[3,2,2,0]
    subject.bodies[:spaceship] = spaceship
    subject.over(3)
    ...
    spaceship.angular_velocity.should == Vector[9, 6, 6, 0]
  end

  it "calculates the distance between objects" do
    earth = Planet.new(:position => Vector[0,0,0,0])
    venus = Planet.new(:position => Vector[5,5,5,0])

    subject.bodies[:earth] = earth
    subject.bodies[:venus] = venus

    earth.distance_of(venus).should == Vector[5,5,5,0]
  end
  ...
  it "assigns the gravitational pull off the earth on me" do
    ...
    me    = Spaceship.new(:mass => 90, :position => Vector[0, 5.83358e6, 0, 0])

    subject.bodies[:earth] = earth
    subject.bodies[:me]    = me

    me.distance_of(earth).should be_vector_like( Vector[0, -5.83358e6, 0, 0])

    subject.over(0)

    me.acceleration.should be_vector_like(Vector[0,-9.8, 0, 0])
  end
  ...
  it "should assign the gravitational pull off the earth on the moon so that it makes a full rotation in 27 days" do
    ...
    earth = Planet.new(:mass => 5e24)
    moon  = Moon.new(:mass => 7.3477e22,:position => Vector[4e8,0,0,0], :velocity => Vector[0, 0, 1022, 0])

    subject.add_bodies(:earth => earth, :moon => moon)
    subject.over(0)

    moon.acceleration.should be_vector_like(Vector[-0.008, 0, 0, 0])
    moon.velocity.should be_vector_like(Vector[0, 0, 1022, 0])

    old_position_moon = moon.matrix.row_vectors.last
